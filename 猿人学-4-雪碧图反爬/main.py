import execjs
import requests
from bs4 import BeautifulSoup


# 获取j_key，这个值用于提取页面数据，判断需要隐藏的img
def get_j_key(key, val):
    with open('hex.js', 'r', encoding='utf-8') as f:
        js = f.read()
    ctx = execjs.compile(js)
    return ctx.call('j_key', key, val)


# 提取数据，移除所有应当在页面中display：none的数据
def extract_img_attributes(html_string, j_key):
    # 解析HTML
    soup = BeautifulSoup(html_string, 'html.parser')
    # 修正j_key
    j_key = j_key.replace('.', '')

    results = []
    # 遍历所有的td标签
    for td in soup.find_all('td'):
        result = []
        results.append(result)
        # 在每个td中查找所有的img标签
        for img in td.find_all('img'):
            # 提取src和style属性
            src = img.get('src', '')  # 如果不存在src属性则返回空字符串
            style = img.get('style', '')
            className = img.get('class', '')[1]
            if className == j_key:
                # print("className: ", className, "等于j_key, 跳过")
                continue

            # 将结果添加到列表中，使用字典形式
            result.append({'src': src, 'class': className, 'style': style})

    return results


# 根据style属性计算得出position属性
def sort_images(td):
    new_td = []
    for i in range(0, len(td)):
        img = td[i]
        style = img['style']
        style = float(style.replace('left:', '').replace('px', ''))
        offset = int(style / 11.5)
        # print("style: ", style, "offset: ", offset)
        position = i + offset + 1
        new_td.append({'src': img['src'], 'class': img['class'], 'style': style, 'position': position})
    return new_td


# 批量计算position
def sort_tds(tds):
    new_tds = []
    for td in tds:
        new_td = sort_images(td)
        new_tds.append(new_td)
    return new_tds


# 计算图片路径到数字的映射，写入src属性中
def img_src_to_num(td):
    num_to_img = {
        0: "data:image/png;base64,",
        1: "data:image/png;base64,",
        2: "data:image/png;base64,",
        3: "data:image/png;base64,",
        4: "data:image/png;base64,",
        5: "data:image/png;base64,",
        6: "data:image/png;base64,",
        7: "data:image/png;base64,",
        8: "data:image/png;base64,",
        9: "data:image/png;base64,"
    }
    img_to_num = {v: k for k, v in num_to_img.items()}
    for img in td:
        src = img['src']
        img['src'] = f'src_img_{img_to_num[src]}'
    return td


# 批量计算路径到数字的映射
def imgs_src_to_nums(tds):
    for td in tds:
        img_src_to_num(td)
    return tds


# 渲染并输出所有的td
def render_tds(tds):
    for td in tds:
        string = 'td: \n'
        for img in td:
            string += 'img: ' + str(img) + '\n'
        string += '\n\n'
        print(string)


# 将td中的img数据解析为数字
def parse_td_images(td):
    """
    将td中的img数据解析为数字字符串
    :param td: 包含图片信息的字典列表
    :return: 拼接后的数字字符串
    """
    # 按position排序
    sorted_images = sorted(td, key=lambda x: x['position'])

    # 提取并拼接数字
    return int(''.join(
        img['src'].split('_')[-1]  # 从src中提取最后一段数字
        for img in sorted_images
    ))


# 将tds解析为数字数组
def parse_tds(tds):
    return [parse_td_images(td) for td in tds]


headers = {
    "accept": "application/json, text/javascript, */*; q=0.01",
    "accept-language": "zh-CN,zh;q=0.9",
    "cache-control": "no-cache",
    "pragma": "no-cache",
    "priority": "u=0, i",
    "referer": "https://match.yuanrenxue.cn/match/4",
    "sec-ch-ua": "\"Microsoft Edge\";v=\"135\", \"Not-A.Brand\";v=\"8\", \"Chromium\";v=\"135\"",
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": "\"Windows\"",
    "sec-fetch-dest": "empty",
    "sec-fetch-mode": "cors",
    "sec-fetch-site": "same-origin",
    "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36 Edg/135.0.0.0",
    "x-requested-with": "XMLHttpRequest"
}
cookies = {
    "sessionid": "iw0wn8sut8pw4gevkegr2zpkrmxi2w1t",
    "Hm_lvt_c99546cf032aaa5a679230de9a95c7db": "1745738875,1745807356",
    "no-alert3": "true",
    "tk": "-7855400030979937383",
    "Hm_lvt_9bcbda9cbf86757998a2339a0437208e": "1745739021,1745746027,1745807358",
    "HMACCOUNT": "4CC429B8CF3F7B95",
    "Hm_lpvt_9bcbda9cbf86757998a2339a0437208e": "1745828841",
    "Hm_lpvt_c99546cf032aaa5a679230de9a95c7db": "1745829430"
}
url = "https://match.yuanrenxue.cn/api/match/4"


total = 0
for i in range(1, 6):
    params = {
        "page": i
    }
    response = requests.get(url, headers=headers, cookies=cookies, params=params, verify=False)
    res_json = response.json()
    # print(response.content)
    key = res_json["key"]
    value = res_json["value"]
    j_key = get_j_key(key, value)
    print(j_key)

    info = response.json()["info"]
    tds = extract_img_attributes(info, j_key)
    tds = sort_tds(tds)
    tds = imgs_src_to_nums(tds)
    # render_tds(tds)
    result = parse_tds(tds)
    print(result)
    total += sum(result)
    print("total: ", total)

print("全部5页值的总和:", total)

